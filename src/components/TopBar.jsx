      ...
      <div className="top-bar-right">
        {!editMode ? (
          <div className="ql-bar">
            {links.map(l => (
              <a key={l.id} href={l.url} target="_blank" rel="noreferrer" className="ql-chip" style={chipStyle}>
                <span>{l.emoji}</span>{l.label}
              </a>
            ))}
            <button className="btn-icon" onClick={() => setEditMode(true)} title="Edit links" style={{ padding: '4px 8px', ...chipStyle }}>
              <Edit2 size={12} />
            </button>
          </div>
        ) : (
            ...
            {links.map(l => (
              editId === l.id ? (
                <div key={l.id} style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  ...
                  <button className="btn-icon" onClick={saveEdit} style={chipStyle}><Check size={11}/></button>
                  <button className="btn-icon" onClick={()=>setEditId(null)} style={chipStyle}><X size={11}/></button>
                </div>
              ) : (
                <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                  <span className="ql-chip" style={{ cursor: 'default', ...chipStyle }}>{l.emoji} {l.label}</span>
                  <button className="btn-icon" onClick={()=>startEdit(l)} style={{padding:3,...chipStyle}}><Edit2 size={10}/></button>
                  <button className="btn-icon" onClick={()=>deleteLink(l.id)} style={{padding:3,...chipStyle}}><X size={10}/></button>
                </div>
              )
            ))}
            {showAdd ? (
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                ...
                <button className="btn-icon" onClick={addLink} style={chipStyle}><Check size={11}/></button>
                <button className="btn-icon" onClick={()=>setShowAdd(false)} style={chipStyle}><X size={11}/></button>
              </div>
            ) : (
              <button className="btn-icon" onClick={()=>setShowAdd(true)} style={chipStyle}><Plus size={12}/></button>
            )}
            <button className="btn-icon" onClick={()=>{setEditMode(false);setEditId(null);setShowAdd(false)}} style={{color:'var(--accent)',...chipStyle}}>
              <Check size={13}/>
            </button>
          </div>
        )}
        ...
        <div ref={themeRef} style={{ position: 'relative' }}>
          <button className="btn-icon" onClick={()=>setShowTheme(s=>!s)} title="Theme" style={chipStyle}>
            <Palette size={15}/>
          </button>
          {showTheme && (
            <div className="theme-panel">
              ...
              <button onClick={toggleTheme} className="btn btn-ghost" style={{width:'100%',justifyContent:'center',gap:8,marginBottom:16,fontSize:13,...chipStyle}}>
              ...
              <div style={{display:'flex',gap:10}}>
                {SCHEMES.map(s => (
                  <button key={s} onClick={()=>setScheme(s)}
                    ...
                    style={{background:SCHEME_COLORS[s],...chipStyle}} title={s}/>
                ))}
              </div>
            </div>
          )}
