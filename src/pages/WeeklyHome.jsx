      ...
      <div className="page-body" style={{display:'flex',flexDirection:'column',gap:16}}>
        ...
        <div className="grid-4">
          ...
          <div className="stat-card">
            ...
            {editSemDate
              ?<div style={{display:'flex',gap:4,marginTop:4}}><input type="date" style={{...inputSm,flex:1}} value={semDate} onChange={e=>setSemDate(e.target.value)}/><button className="btn-icon" style={{padding:4}} onClick={()=>setEditSemDate(false)}>✓</button></div>
          ...
          <div className="stat-card"><div className="stat-label">Upcoming</div><div className="stat-value" style={{color:'var(--amber)',fontSize:22}}>{upcoming.length}</div><div className="stat-sub">assignments &amp; plans</div></div>
        </div>
          ...
          <div className="card home-tasks-col">
            ...
            {activeTasks.map(task=>(
              <TaskRow key={task.id} task={task}
                courseColors={COURSE_COLORS} courseOptions={courseOptions}
                  ...
                  return (
                    <div key={item.id||i} style={{
                      ...
                      <div style={{fontSize:isFirst?12:10,fontWeight:700,color:due?.color||color}}>
                        {due?.label||planDate||'—'}
                        {item._type==='assignment'&&item.courseName&&<span style={{fontSize:9,color:'var(--text-3)',fontWeight:400,marginLeft:6}}>{item.courseName}</span>}
                      </div>
                    </div>
                  )
                })
