...
import { Plus, Trash2, Edit2, Check, X, ChevronDown, ChevronUp, ArrowUp, ArrowDown, BookOpen } from 'lucide-react'
...
function daysFromNow(dateStr) {
  ...
  const diff = Math.ceil((new Date(dateStr + 'T23:59:00') - new Date()) / 86400000)
  return diff
}

// ── Assignments due this week for a course ───────────────────────
...
// ── Grade calculation for a course ──────────────────────────────
...
function Modal({ title, onClose, children }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:900, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}
      ...
      <div className="card" style={{ position:'relative', zIndex:1, width:'100%', maxWidth:520, maxHeight:'90vh', overflow:'auto', padding:0 }}
...
// ── Assignment edit form (used in modal) ─────────────────────────
function AssignmentForm({ form, onChange, onSave, onCancel, saving }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
        <div style={row}>
          <label style={lbl}>Start date <span style={{fontWeight:400,fontSize:10}}>(shows on calendar)</span></label>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
        <div style={row}>
          <label style={lbl}>Due time <span style={{fontWeight:400,fontSize:10}}>(optional)</span></label>
      ...
      {/* Priority — hidden label, shown as pill buttons */}
        ...
        <div style={{ display:'flex', gap:6, marginTop:6, flexWrap:'wrap' }}>
          {PRIORITY.map(p=>(
            <button key={p.key} onClick={()=>onChange({...form,priority:p.key})} style={{
              ...
              color: form.priority===p.key ? p.color : 'var(--text-3)',
              transition:'all .15s',
            }}>{p.label}</button>
          ))}
        </div>
      ...
      {/* Score tracking */}
      <div style={{ marginBottom:14 }}>
        <label style={lbl}>Grade <span style={{fontWeight:400,fontSize:10}}>(fill in after graded)</span></label>
        <div style={{ display:'flex', gap:8, alignItems:'center', marginTop:6 }}>
          ...
          {form.score && form.maxScore && (
            <span style={{fontSize:13,fontWeight:700,color:Number(form.score)/Number(form.maxScore)>=.9?'var(--green)':Number(form.score)/Number(form.maxScore)>=.7?'var(--amber)':'var(--coral)'}}>
...
export default function Courses({ onDataChange }) {
  ...
  const [openCourseId, setOpenCourseId] = useState(null)  // one accordion open at a time
  ...
  // Assignment modal
  const [addModal,  setAddModal]  = useState(null)   // courseId
  const [editModal, setEditModal] = useState(null)   // { termId, courseId, assignId }
  const [addForm,   setAddForm]   = useState(BLANK_ASSIGN)
  const [editForm,  setEditForm]  = useState(BLANK_ASSIGN)

  // Notes expand state per assignment
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setJumpAssignId(null), 2500)
  ...
  const addCourse = () => {
    ...
    const newId = uid()
    updateTerms(ts => ts.map(t => t.id!==activeTermId ? t : {
      ...t, courses: [...t.courses, { ...newCourse, id:newId, assignments:[] }]
    }))
    setNewCourse(BLANK_COURSE); setShowAddCourse(false)
    setOpenCourseId(newId)
  }
  const deleteCourse = (termId, courseId) => {
    if (!confirm('Delete this course and all its assignments?')) return
    updateTerms(ts => ts.map(t => t.id!==termId ? t : { ...t, courses:t.courses.filter(c=>c.id!==courseId) }))
  ...
  const moveCourse = (termId, courseId, dir) => {
    updateTerms(ts => ts.map(t => {
      ...
      const cs=[...t.courses], idx=cs.findIndex(c=>c.id===courseId), to=idx+dir
  ...
  // ── Assignment ops ───────────────────────────────────────────
  const saveAddAssignment = (termId, courseId) => {
    if (!addForm.title.trim() || !addForm.due) return
    updateTerms(ts => ts.map(t => t.id!==termId ? t : {
      ...t, courses: t.courses.map(c => c.id!==courseId ? c : {
        ...c, assignments: [...c.assignments, {...addForm, id:uid()}]
      })
    }))
    setAddForm(BLANK_ASSIGN); setAddModal(null)
  }
  const saveEditAssignment = () => {
    if (!editModal) return
    const { termId, courseId, assignId } = editModal
    updateTerms(ts => ts.map(t => t.id!==termId ? t : {
      ...t, courses: t.courses.map(c => c.id!==courseId ? c : {
        ...c, assignments: c.assignments.map(a => a.id!==assignId ? a : {...a,...editForm})
      })
    }))
    setEditModal(null)
  }
  const toggleDone = (termId, courseId, assignId, current) => {
    updateTerms(ts => ts.map(t => t.id!==termId ? t : {
      ...t, courses: t.courses.map(c => c.id!==courseId ? c : {
        ...c, assignments: c.assignments.map(a => a.id!==assignId ? a : {...a, status:current==='Done'?'To do':'Done'})
      })
    }))
  }
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Track every assignment, due date, and grade</div>
        </div>
        ...
        {activeTerm && (
          <>
            ...
            {activeTerm.courses.map((course, courseIdx) => {
              ...
              // Sort: undone by due date (soonest first), Done items sink to bottom
              ...
              return (
                <div key={course.id} className="card" style={{ padding:0, overflow:'hidden', borderLeft:`4px solid ${course.color}` }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:10, padding:'14px 16px', cursor:'pointer' }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                        ...
                        {weekCount>0 && (
                          <span style={{ fontSize:10, padding:'2px 7px', borderRadius:20, background:'var(--coral-dim)', color:'var(--coral)', fontWeight:700, border:'1px solid var(--coral)' }}>
                    ...
                    <div style={{ display:'flex', gap:4, flexShrink:0 }} onClick={e=>e.stopPropagation()}>
                      ...
                      <Tooltip text="Delete course">
                        <button className="btn-icon" style={{padding:4,color:'var(--coral)'}} onClick={()=>deleteCourse(activeTerm.id,course.id)}><Trash2 size={12}/></button>
                      </Tooltip>
                      <Tooltip text="Add assignment">
                        <button className="btn-icon" style={{padding:4,color:'var(--accent)'}} onClick={()=>{setAddForm(BLANK_ASSIGN);setAddModal(course.id)}}><Plus size={12}/></button>
                      </Tooltip>
                    </div>
                  ...
                  {/* Progress bar */}
                  {total>0&&(
                    <div style={{height:2,background:'var(--glass-border)',margin:'0 16px 0'}}>
                      <div style={{height:'100%',background:course.color,width:`${(doneCount/total)*100}%`,borderRadius:2,transition:'width .4s'}}/>
                    </div>
                  )}
                  ...
                  {editCourseId===course.id&&(
                    <div style={{padding:'14px 16px',borderTop:'1px solid var(--glass-border)',display:'flex',flexDirection:'column',gap:10}}>
                      ...
                      <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
                        ...
                        {COURSE_COLORS.map(c=>(
                          <button key={c} onClick={()=>setEditCourseForm(f=>({...f,color:c}))} style={{width:22,height:22,borderRadius:'50%',background:c,border:`2px solid ${editCourseForm.color===c?'white':'transparent'}`,cursor:'pointer'}}/>
                        ))}
                      </div>
                      <div style={{display:'flex',gap:8}}>
                        ...
                        return (
                          <div key={a.id} id={`assign-${a.id}`} style={{
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 14px', flexWrap:'wrap' }}>
                              ...
                              {/* Checkbox */}
                              <button onClick={()=>toggleDone(activeTerm.id,course.id,a.id,a.status)} style={{
                                background:'none', border:'none', cursor:'pointer', padding:2,
                                color: isDone?'var(--green)':'var(--glass-border)',
                              ...
                              <div style={{ flex:1, minWidth:0 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap', marginBottom:3 }}>
                                  ...
                                  {a.submissionType&&a.submissionType!=='Canvas'&&<span style={{fontSize:10,padding:'1px 6px',borderRadius:20,background:'var(--glass-bg-2)',color:'var(--accent)',border:'1px solid var(--accent)44'}}>{a.submissionType}</span>}
                              ...
                              <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                                <Tooltip text="Edit">
                                  <button className="btn-icon" style={{padding:5}} onClick={()=>{setEditModal({termId:activeTerm.id,courseId:course.id,assignId:a.id});setEditForm({title:a.title,type:a.type||'Essay',due:a.due||'',dueTime:a.dueTime||'',startDate:a.startDate||'',status:a.status,priority:a.priority||'none',notes:a.notes||'',score:a.score||'',maxScore:a.maxScore||'100',submissionType:a.submissionType||'Canvas'})}}><Edit2 size={12}/></button>
                                </Tooltip>
                                <Tooltip text="Delete">
                                  <button className="btn-icon" style={{padding:5,color:'var(--coral)'}} onClick={()=>deleteAssign(activeTerm.id,course.id,a.id)}><Trash2 size={12}/></button>
                                </Tooltip>
                              </div>
                            </div>
                            ...
                            {a.notes&&(
                              <div style={{padding:'0 14px 10px 42px'}}>
                                <button onClick={()=>setOpenNotes(n=>({...n,[a.id]:!n[a.id]}))} style={{
            ...
            {showAddCourse ? (
              <div className="card">
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                  ...
                  <input style={inp} placeholder="Days (Mon/Wed)" value={newCourse.days} onChange={e=>setNewCourse(f=>({...f,days:e.target.value}))}/>
      ...
      {addModal && (
        <Modal title="New assignment" onClose={()=>setAddModal(null)}>
          <AssignmentForm
            ...
            onSave={()=>saveAddAssignment(activeTerm.id,addModal)}
